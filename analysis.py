...
train['emotions'] = train['emotions'].apply(lambda x: [int(_i) for _i in x.split(',')])     # 去掉','，留下0 ## lambda x 函数用于取需要复杂运算确定的列
...
class RoleDataset(Dataset):
    def __init__(self, tokenizer, max_len, mode='train'):
        ...
        self.labels=self.data[target_cols].to_dict('records')   # 获取各行的target_cols标签的值
    ...
    def __getitem__(self, index):
        ...
        for label_col in target_cols:
            sample[label_col] = torch.tensor(label[label_col]/3.0, dtype=torch.float)    # 根据数据直接创建tensor
        return sample
